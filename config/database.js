...
const initDb = async () => {
  try {
    ...
    // Create household_items table with purchase_counter
    await pool.query(`
    ...
    // Add purchase_counter column if it doesn't exist
    const [columns] = await pool.query(`
      SELECT COLUMN_NAME 
      FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_NAME = 'household_items' 
      AND COLUMN_NAME = 'purchase_counter'
    `);

    if (columns.length === 0) {
      await pool.query(`
        ALTER TABLE household_items 
        ADD COLUMN purchase_counter INT DEFAULT 0
      `);
    }
