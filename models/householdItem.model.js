    ...
    try {
      const [result] = await db.query(
        `INSERT INTO household_items 
         (household_id, item_id, location, item_photo, price, expiration_date, purchase_counter) 
         VALUES (?, ?, ?, ?, ?, ?, 1)`,
        [householdId, itemId, location, itemPhoto, price, expirationDate]
      );
  ...
  static async moveItemToHouse(householdItemId, householdId, price, expirationDate) {
    try {
      const [result] = await db.query(
        ...
        [price, expirationDate, householdItemId, householdId]
      );
      return result.affectedRows > 0;
    } catch (error) {
      throw error;
